#!/usr/bin/env python3
"""
Test TP/SL fix for PYTHUSDT position
"""
import asyncio
import sys
import os
...
async def test_pythusdt_tpsl_fix():
    """Test TP/SL fix for PYTHUSDT position"""
    try:
        from app.bybit.client import get_bybit_client
        from app.core.intelligent_tpsl_fixed import set_intelligent_tpsl_fixed
        
        print("🎯 Testing TP/SL Fix for PYTHUSDT Position...")
        print("=" * 50)
        
        client = get_bybit_client()
        
        # Get current position
        positions_response = await client.get_positions('linear', 'PYTHUSDT')
            ...
            for position in positions:
                ...
                if size > 0:
                    ...
                    result = await set_intelligent_tpsl_fixed(
                        ...
                        trade_id="pythusdt_tpsl_fix_test"
                    )
                    
                    print(f"✅ TP/SL Result: {result}")
                    return
                    
        print("❌ No active PYTHUSDT position found")
...
if __name__ == "__main__":
    asyncio.run(test_pythusdt_tpsl_fix())
