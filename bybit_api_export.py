...
RECV_WINDOW = "30000"  # Use same window as the bot
        ...
        try:
            while True:
                ...
                if j.get("retCode") == 0:
                    ...
                    out.extend(trades)
                    print(f"    Found {len(trades)} trades in this batch")
...
def main(start_iso: str=None, end_iso: str=None, outfile="bybit_demo_trades.xlsx"):
    ...
    print(f"🚀 Bybit Demo Trade Export")
    ...
    for c in cats:
        ...
        print(f"✅ Found {len(rows)} {c} trades total")
    ...
    # Create Excel file
    if all_data:
        print(f"📝 Creating Excel file: {outfile}")
        ...
        if "execTime_Stockholm" in combined_df.columns:
            print("\n🕐 Most Recent Trades:")
            recent = combined_df.nlargest(5, "execTimeMs")
        ...
        print(f"⚠️  No trades found. Creating empty file: {outfile}")
        with pd.ExcelWriter(outfile, engine="openpyxl") as xlw:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        ...
        print("Usage: python bybit_api_export.py [<start ISO> <end ISO>]")
