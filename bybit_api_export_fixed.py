...
def sign_v5_get(params: dict) -> dict:
    """v5 signature for GET requests: timestamp + api_key + recv_window + queryString (sorted)"""
    ...
    for attempt in range(max_retries):
        try:
            while True:
                ...
                headers = sign_v5_get(p)
                print(f"Fetching {category} trades (attempt {attempt + 1})...")
                
                r = requests.get(API_BASE+ENDPOINT, params=p, headers=headers, timeout=30)
                r.raise_for_status()
                j = r.json()
                
                print(f"API Response: {j.get('retCode')} - {j.get('retMsg', 'OK')}")
                
                if j.get("retCode") == 0:
                    res = (j.get("result") or {}) 
                    trades = res.get("list") or []
                    out.extend(trades)
                    print(f"Found {len(trades)} {category} trades in this batch")
                    ...
                    time.sleep(2)
                    break
                elif j.get("retCode") == 10004:  # Signature error
                    print(f"Signature error, retrying... (attempt {attempt + 1})")
                    time.sleep(2)
                    break
                else:
                    print(f"API Error: {j}")
...
def main(start_iso: str=None, end_iso: str=None, outfile="bybit_demo_trades_api.xlsx"):
    ...
    for c in cats:
        print(f"\n=== Fetching {c.upper()} trades ===")
        rows = fetch_execs(c, sm, em)
        print(f"Total {c} trades found: {len(rows)}")
    ...
    if all_data:
        ...
        # Show recent trades
        if "execTime_Stockholm" in combined_df.columns:
            print("\n🕒 Most Recent Trades:")
            recent = combined_df.nlargest(5, "execTimeMs")[["symbol", "side", "execQty", "execPrice", "execTime_Stockholm"]]
            for _, trade in recent.iterrows():
                print(f"  {trade['symbol']} {trade['side']} {trade['execQty']} @ {trade['execPrice']} ({trade['execTime_Stockholm']})")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        ...
        print("Usage: python bybit_api_export_fixed.py [<start ISO> <end ISO>]")
